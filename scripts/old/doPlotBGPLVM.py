...
def main(argv):
    ...
    variational_vars = gplvm.variational_var.detach().numpy()[:,latents_to_plot]
    x_mog_density = np.linspace(variational_means[:,0].min(), variational_means[:,0].max())
    y_mog_density = np.linspace(variational_means[:,1].min(), variational_means[:,1].max())
    mog_density = stats.get_density_mixture_of_2DIndependentGaussians(x=x_mog_ensity, y=y_mog_ensity,
                                                                      means=variational_means,
                                                                      variances=variational_vars)

    plt.figure()
    plt.imshow(mog_density, extent=[x_mog_density.min(), x_mog_density.max(), y_mog_density.max(), y_mog_density.min()], cmap="gray")
